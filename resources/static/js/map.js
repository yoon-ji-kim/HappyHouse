...
function displayMarkers(places) { //아파트 json배열로 호출
	...
	removeMarker();
	for(var i=0; i<places.length; i++) { //아파트 목록만큼 for문
		var placePosition = new kakao.maps.LatLng(places[i].lat, places[i].lng); //위도와 경도 얻어내기, DB에 있는 내용
		...
		(function (marker, title, code, place) {
		    ...
		    kakao.maps.event.addListener(map, "click", function () { //맵클릭시 , 
